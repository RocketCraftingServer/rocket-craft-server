...
if (config.UseAuthentication && config.UseHTTPS) {
  ...
  console.log(
    "ERROR: Trying to use authentication without using HTTPS, this is not allowed and so authentication will NOT be turned on, please turn on HTTPS to turn on authentication"
  );
}
...
try {
  if (typeof config.publicIp != "undefined") {
		...
		console.info("Server signaling public address => ", serverPublicIp)
  }
  ...
  app.use(function (req, res, next) {
    if (!req.secure) {
      if (req.get("Host")) {
        ...
        console.log(
          `ERROR unable to get host name from header. Requestor ${
            req.ip
...
function readJsonMsg(consumed) {
  ...
  if (proxyBuffer.length < consumed + 4) return [0, ""];
  let msgSize = proxyBuffer.readUInt32LE(consumed);
  consumed += 4;
  if (proxyBuffer.length < consumed + msgSize) return [0, ""];
...
function handleProxyMessage(socket) {
  // msgId
  if (proxyBuffer.length == 0) return false;
  let msgId = proxyBuffer.readUInt8(0);
  let consumed = 1;

  // clientId
  if (proxyBuffer.length < consumed + 4) return false;
  let clientId = proxyBuffer.readUInt32LE(consumed);
  consumed += 4;

  let client = clients.find(function (c) {
  ...
  if (!client) {
    ...
    console.error(
      `proxy message ${msgId}: client ${clientId} not found. Check proxy->cirrus protocol consistency`
    );
  }
  ...
  switch (msgId) {
    ...
    case EProxyToCirrusMsg.iceCandidate:
      ...
      if (localConsumed == 0) return false;
...
function handleNewClient(ws) {
  ...
  console.log(
    `client ${clientId} (${ws.request.connection.remoteAddress}) connected`
  );
  ...
  switch (userConfig.emitData) {
    case "ArrayBuffer": {
      ...
      ws.on("webrtc-stats", function (stats) {
        console.log(
          `Received webRTC stats from player ID: ${clientId} \r\n${JSON.stringify(
            stats
          )}`
        );
      });
