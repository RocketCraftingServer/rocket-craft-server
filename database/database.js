...
class MyDatabase {
	...
	async populateDatabase() {
		...
		// Console log works only for return obj
			...
			MongoClient.connect(
				...
				function(error, db) {
					...
					dbo.collection("users").findOne({email: user.email, token: user.token}, function(err, result) {
						...
						if(result !== null) {
							dbo.collection("users").updateOne({email: user.email}, {$set: {confirmed: true, points: 1000}}, function(err, result) {
								if(err) {
					...
					dbo.collection("users").findOne({email: user.email, confirmed: true}, {}, function(err, result) {
						...
						if(result !== null) {
							...
							if(pass == user.password) {
								...
								// handle bad cert
								console.warn("login.bad.password -> ", user.password);
								const userData = {
