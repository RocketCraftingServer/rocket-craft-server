...
class CreateDatabaseCollections {
	...
	createCollections() {
		...
		const r0 = this.createAntiHackCollection();
	...
	createAntiHackCollection() {
		...
		return new Promise((resolve) => {
			MongoClient.connect(this.config.getDatabaseRoot,{useNewUrlParser: true, useUnifiedTopology: true},
				function(error, db) {
					if(error) {console.warn("MyDatabase  error:" + error);
						return;
					}
					const dbo = db.db(databaseName);
					if(!dbo.collection("visitors")) {
						dbo.createCollection("visitors").createIndex({useragent: 1}, {unique: true});
						dbo.createCollection("visitors").createIndex({description: 1}, {unique: true});
						resolve("Collections visitors created.")
					} else {
						resolve("Collections visitors already exist.")
					}
				})
		})
	}
